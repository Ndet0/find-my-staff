...
export default function FacilityRelationshipsClient() {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    fetchRelationships();
  }, [user, supabase, dimensions.width, dimensions.height]);
  ...
  const getNodeColor = (node: Node) => {
    if (node.type === "facility") return "#166534";
    return "#22c55e";
  };
  const getNodeStroke = (node: Node) => {
    if (node.type === "facility") return "#14532d";
    return "#16a34a";
  };

  const getLinkColor = (link: Link) => {
    return link.employment_type === "1099" ? "#a855f7" : "#3b82f6";
  };
